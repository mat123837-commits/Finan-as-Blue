  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center px-1">
          ...
          {/* Botão de menu caso o usuário queira acessar outras áreas do app */}
      ...
      {/* SEÇÃO BENTO BOX MOBILE */}
      <div className="grid grid-cols-2 gap-4">
        
        {/* LIQUIDEZ LIVRE (DESTAQUE) */}
        <div className="col-span-2 bg-[#13312A] rounded-[2.5rem] p-8 text-[#F6E9CA] relative overflow-hidden shadow-2xl shadow-[#13312A]/10">
            ...
            <div className="relative z-10">
                ...
                <p className="text-[10px] mt-4 text-[#C69A72] font-bold uppercase tracking-widest leading-relaxed">
                    Saldo disponível após <br/> todas as contas do mês
                </p>
            </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-[#13312A]/5 flex flex-col justify-between min-h-[140px]">
            ...
            <div>
                ...
                <p className="text-xl font-bold text-[#13312A] font-serif">
                    {showValues ? formatCurrency(netBalance) : 'R$ ••••'}
                </p>
            </div>
        </div>

        {/* LIMITE CARTÃO (CICLÁVEL) */}
        <div 
          onClick={cycleCard}
          className="bg-white rounded-[2rem] p-6 shadow-sm border border-[#13312A]/5 flex flex-col justify-between min-h-[140px] active:scale-95 transition-all cursor-pointer"
        >
            ...
            <div>
                <p className="text-[10px] font-bold text-[#155446]/40 uppercase tracking-widest mb-1 truncate">
                    {activeCard ? activeCard.name : 'Cartão'}
                </p>
                <p className="text-xl font-bold text-[#13312A] font-serif">
                    {showValues && activeCard ? formatCurrency(getCardAvailableLimit(activeCard.id)) : 'R$ ••••'}
                </p>
            </div>
        </div>
      </div>

      {/* BOTÕES DE LANÇAMENTO GIGANTES */}
      <div className="grid grid-cols-2 gap-4">
        <button
          onClick={() => onOpenTransaction('expense')}
          className="h-24 rounded-[2.5rem] bg-[#9F3E34] text-[#F6E9CA] flex items-center justify-center gap-4 shadow-lg shadow-[#9F3E34]/20 active:scale-95 transition-all group"
        >
        ...
        <button
          ...
          className="h-24 rounded-[2.5rem] bg-[#155446] text-[#F6E9CA] flex items-center justify-center gap-4 shadow-lg shadow-[#155446]/20 active:scale-95 transition-all group"
        >
          <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
            ...
            {recentTransactions.map(t => (
               <div key={t.id} className="bg-white p-5 rounded-[2.2rem] shadow-sm border border-[#13312A]/5 flex items-center justify-between active:bg-[#F6E9CA]/20 transition-all">
                  <div className="flex items-center gap-4">
                     <div className={`w-11 h-11 rounded-[1.2rem] flex items-center justify-center ${t.type === 'income' ? 'bg-[#155446]/10 text-[#155446]' : 'bg-[#9F3E34]/10 text-[#9F3E34]'}`}>
                  ...
                  <span className={`font-bold font-serif text-base ${t.type === 'income' ? 'text-[#155446]' : 'text-[#13312A]'}`}>
