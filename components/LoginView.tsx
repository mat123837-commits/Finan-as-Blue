...
import { 
  ...
  PieChart as PieIcon, ArrowUpRight, ShieldCheck, Heart, 
  Car, Home, Zap, ChevronDown, Sparkles, Layers
} from 'lucide-react';
...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="bg-[#F6E9CA] h-screen overflow-y-auto no-scrollbar scroll-smooth">
      ...
      {/* --- SECTION 1: HERO & LOGIN (FIRST FOLD) --- */}
      <section ref={loginSectionRef} className="min-h-screen flex flex-col lg:flex-row overflow-hidden relative">
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-24 bg-[#F6E9CA] relative z-50">
          <div className="w-full max-w-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                 ...
                 <div className="space-y-2">
                    <label className="text-xs font-bold text-[#13312A] uppercase tracking-widest ml-1">Senha</label>
                    <div className="relative group">
                       ...
                       <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-white border-2 border-transparent focus:border-[#C69A72] hover:bg-white/80 rounded-[1.5rem] py-5 pl-14 pr-14 text-[#13312A] font-bold shadow-sm outline-none transition-all placeholder:text-[#155446]/20" required minLength={6} />
                 ...
                 <button type="submit" disabled={isLoading} className={`w-full py-5 rounded-[1.5rem] font-bold text-xl shadow-2xl flex items-center justify-center gap-4 transition-all duration-300 transform active:scale-[0.98] mt-4 font-serif ${isLoading ? 'bg-[#155446]/50 text-white/50 cursor-not-allowed' : 'bg-[#13312A] text-[#C69A72] shadow-[#13312A]/40 hover:bg-black hover:-translate-y-1'}`}>
                    {isLoading ? <div className="w-7 h-7 border-3 border-[#C69A72] border-t-transparent rounded-full animate-spin" /> : <>{isSignUp ? 'Criar Minha Conta' : 'Acessar Meu Dashboard'} <ArrowRight size={22} /></>}
                 </button>

                 <div className="pt-8 text-center border-t border-[#13312A]/5 mt-8">
                    <p className="text-sm text-[#155446]/50 mb-3">{isSignUp ? 'Já possui cadastro no Fingreen?' : 'Ainda não possui uma conta?'}</p>
                    <button type="button" onClick={() => { setIsSignUp(!isSignUp); setErrorMsg(''); setSuccessMsg(''); setShowSuggestions(false); }} className="text-[#13312A] font-bold text-base hover:text-[#C69A72] transition-colors inline-flex items-center gap-2 group">
                       {isSignUp ? 'Fazer Login Agora' : 'Comece a usar Grátis'} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                    </button>
                 </div>
              </form>
          </div>
      ...
      {/* --- SECTION 2: THE PHILOSOPHY (CLARITY) --- */}
      <section className="py-24 lg:py-40 px-6 lg:px-24 bg-white text-[#13312A]">
         <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-16 lg:gap-32">
            <div className="lg:w-1/2">
               <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#F6E9CA] rounded-full text-[#13312A] text-xs font-bold uppercase tracking-widest mb-8">
                  <Sparkles size={14} /> Previsibilidade Total
               </div>
               <h3 className="text-5xl lg:text-7xl font-serif font-bold leading-tight mb-8">
                  Chega de <br/>
                  <span className="text-[#C69A72]">contas de cabeça.</span>
               </h3>
               <p className="text-xl text-[#155446]/70 leading-relaxed font-medium mb-12">
                  Planilhas tradicionais focam no que você gastou. <br/>
                  O <strong>Fingreen</strong> foca no que você pode gastar amanhã. <br/>
                  Nosso indicador de <strong>Liquidez Livre</strong> subtrai automaticamente suas faturas e gastos fixos do seu saldo real, entregando clareza mental instantânea.
               </p>
      ...
      {/* --- SECTION 3: ECOSYSTEM SHOWCASE (Lifestyle Management) --- */}
      <section className="py-24 lg:py-40 px-6 lg:px-24 bg-[#13312A] text-[#F6E9CA]">
         <div className="max-w-6xl mx-auto">
            <div className="text-center mb-24">
               <h3 className="text-4xl lg:text-6xl font-serif font-bold mb-6">Controle seu estilo de vida.</h3>
               <p className="text-lg text-[#F6E9CA]/60 max-w-2xl mx-auto">
                  Muito mais que uma carteira. Um ecossistema completo para gerenciar os pilares que realmente importam no seu dia a dia.
               </p>
            </div>
      ...
      <section className="py-24 lg:py-40 px-6 lg:px-24 bg-[#FFFDF5]">
         <div className="max-w-6xl mx-auto flex flex-col lg:flex-row-reverse items-center gap-16 lg:gap-32">
            <div className="lg:w-1/2">
               <h3 className="text-5xl lg:text-7xl font-serif font-bold text-[#13312A] leading-tight mb-8">
                  Privacidade <br/>
                  <span className="text-[#C69A72]">por design.</span>
               </h3>
               <div className="space-y-8">
                  <div className="flex gap-6">
                     <div className="shrink-0 w-12 h-12 bg-[#13312A]/5 text-[#13312A] rounded-xl flex items-center justify-center"><ShieldCheck size={24} /></div>
                     <div>
                        <h5 className="font-bold text-[#13312A] mb-1">Criptografia Ponta a Ponta</h5>
                  ...
                  <div className="flex gap-6">
                     <div className="shrink-0 w-12 h-12 bg-[#13312A]/5 text-[#13312A] rounded-xl flex items-center justify-center"><Layers size={24} /></div>
                     <div>
                        <h5 className="font-bold text-[#13312A] mb-1">Modo Privacidade</h5>
                  ...
                  <div className="relative bg-white border border-[#13312A]/5 rounded-[3rem] p-4 shadow-2xl overflow-hidden">
                     ...
                     <div className="p-8">
                        <div className="flex justify-between items-center mb-12">
                           <div>
                              <div className="h-3 w-24 bg-gray-100 rounded-full mb-3"></div>
                              <div className="h-8 w-48 bg-[#13312A] rounded-xl"></div>
                           </div>
                           <div className="w-12 h-12 rounded-full bg-[#C69A72]"></div>
                        </div>
      ...
      <footer className="py-24 lg:py-40 px-6 lg:px-24 bg-[#13312A] text-[#F6E9CA] relative overflow-hidden">
         <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#C69A72] to-transparent opacity-20"></div>
         <div className="max-w-4xl mx-auto text-center relative z-10">
            ...
            <p className="text-xl text-[#F6E9CA]/60 mb-12 max-w-xl mx-auto font-medium">
               Junte-se a centenas de pessoas que redescobriram o prazer de gerenciar seu dinheiro.
            </p>
            <button onClick={scrollToLogin} className="group px-12 py-6 bg-[#C69A72] text-[#13312A] rounded-[2rem] font-bold text-xl shadow-2xl shadow-black/40 hover:scale-105 active:scale-95 transition-all flex items-center gap-4 mx-auto font-serif">
            ...
            <div className="mt-40 pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40">
               <div className="flex items-center gap-3">
                  <Wallet size={20} />
                  <span className="font-bold font-serif">Fingreen Private</span>
               </div>
               <p className="text-xs">© 2025 Fingreen. Todos os direitos reservados.</p>
               <div className="flex gap-6 text-xs font-bold uppercase tracking-widest">
                  <a href="#" className="hover:text-[#C69A72]">Termos</a>
                  <a href="#" className="hover:text-[#C69A72]">Privacidade</a>
               </div>
            </div>
         </div>
      </footer>

      {/* Floating animation keyframes (Global for this view) */}
      <style>{`
        @keyframes float {
